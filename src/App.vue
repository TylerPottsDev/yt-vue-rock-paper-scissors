...
const ResetAll = () => {
	localStorage.clear('wins', wins.value);
	localStorage.clear('losses', losses.value);
	localStorage.clear('draws', draws.value);
	ResetRound();
	wins.value = 0;
	losses.value = 0;
	draws.value = 0;
	winPercentage.value = 0;
};
	...
	<div class="bg-gray-700 text-white text-center min-h-screen flex flex-col">
		...
		<main class="container mx-auto p-6 flex-1">
			...
			<div class="mt-12 text-3xl mb-4"
				>{{ wins }} : {{ draws }} : {{ losses }}</div
			>

			<div class="text-lg">Win rate: {{ Math.round(winPercentage) }}%</div>
			<button
				class="bg-white shadow-lg text-gray-700 font-semibold mt-6 py-2 px-4 rounded-lg transition-colors duration-300 hover:bg-red-500"
				@click="ResetAll"
				>New Game</button
			>
		</main>
